...
这一版重点提升“可体验、可运维、可展示”的完整度：

| 方向 | 更新内容 |
|---|---|
| **国际化界面** | 前端完成 `zh-CN` / `en-US` 双语支持，语义化翻译 Key、语言切换器和 Element Plus 语言同步。 |
| **规则链编辑体验** | 增加画布缩放/拖拽、快捷键、节点搜索、规则校验和调试路径高亮。 |
| **脚本化自动化** | 支持脚本过滤节点；协议/脚本测试中的用户脚本错误会返回清晰校验结果，不再制造噪声 500。 |
| **集成可靠性** | 推送配置校验、签名推送测试、投递参数检查更完整。 |
| **演示与发布质量** | GitHub Actions 加固，CI 增加前端 lint，演示环境自动部署和公网冒烟验证更稳定。 |
