...
**Simple IoT** 是一套面向自托管场景的物联网平台：接设备、建物模型、写协议脚本、编排规则链、处理告警和通知，都可以在一个清晰的控制台里完成。

项目坚持紧凑的运行形态：一个 Spring Boot 应用、一个 Vue 3 控制台，PostgreSQL 存业务数据，InfluxDB 存遥测数据，Docker Compose 完成部署。它适合小团队、集成商和创客直接阅读、二次开发和交付。

## 🌟 本次发版亮点

### v0.3.0 - 驱动 + 数据接入（2026-07-31）

| 方向 | 更新内容 |
|---|---|
| **Webhook 接入** | HMAC-SHA256 签名 HTTP 入口，第三方平台推送数据。每接入独立 token + 密钥，防重放，前端 CRUD + curl 示例 |
| **Modbus TCP** | 按设备定时轮询，寄存器映射配置，多功能码读取，缩放 + 数据类型转换。含 `modbus-sim` 模拟器 |
| **系统监控** | 仪表盘展示真实 CPU / 内存 / 磁盘使用率（替换原来的占位 0%） |
| **UI 审计** | 8 个页面统一到 PageHeader + glass-card + el-empty 空状态，删除 40 行重复 CSS |

### v0.2.0 - 实时化 + 运维增强（2026-07-23）

| 方向 | 更新内容 |
|---|---|
| **实时仪表盘** | WebSocket 推送遥测/告警，新增设备状态网格 + 告警列表两个组件 |
| **定时触发** | 规则引擎支持 cron 定时触发节点 |
| **IM 推送** | 钉钉/企微/飞书 Webhook 通知，前端配置页 |
| **批量操作** | 多设备批量下发指令，逐设备结果反馈 |
| **设备拓扑** | 父子设备关系，网关-子设备场景 |
| **数据保留** | InfluxDB 遥测自动清理（可配置，默认 180 天） |
| **安全** | API 限流、审计日志、告警抑制（5 分钟去重） |
| **文档** | VitePress 新增 8 篇指南（中英双语），190 单测，平台成熟度 75% |

### v0.1.1 - 易用性（2026-06-25）

| 方向 | 更新内容 |

> **一句话** — Simple IoT 是一套能自己部署、能看懂源码、能快速改造成项目形态的轻量物联网控制台。
...
| **产品 & 物模型** | 产品类型、属性、服务、事件，用结构化方式描述设备能力 |
| **协议引擎** | 热加载脚本，支持 **Java / JavaScript / Groovy / Lua**，无需重启 |
| **可视化规则引擎** | 拖拽式规则链编辑器，支持校验、调试路径、脚本过滤和节点复用 |
| **告警中心** | 多级（提示 / 警告 / 严重 / 紧急），活动 / 已清除生命周期 |
| **数据采集** | InfluxDB 3 时序存储，Caffeine 进程内缓存 |
| **通知推送** | 邮件、短信、HTTP 回调、MQTT 转发、设备指令、钉钉/企微/飞书 IM 推送 |
| **仪表盘** | 设备总数、在线统计、系统资源、实时告警、WebSocket 实时推送、设备状态网格 + 告警列表组件 |
| **实时推送** | WebSocket 实时遥测、告警、设备状态变化 |
| **数据保留** | InfluxDB 遥测自动清理（可配置，默认 180 天） |
| **安全** | API 限流（Bucket4j）、审计日志（AOP）、告警抑制（5 分钟去重） |
| **认证权限** | 基于 Sa-Token，细粒度，角色 / 菜单 / 按钮级 |
| **国际化 UI** | Vue 3 控制台，支持 `zh-CN` / `en-US`、亮色 / 暗色 / 跟随系统、PWA 支持 |
...
- [x] **v0.1** — 单节点稳定版、双语文档、CI、在线演示
- [x] **v0.2** — `zh-CN` / `en-US` 双语界面、规则链编辑器优化、演示部署加固
- [ ] **v0.3** — 物模型导入导出、设备分组
- [ ] **v0.4** — 自定义数据大屏（拖拽组件）
- [ ] **v0.5** — OTA 升级流程、边缘网关打包
- [ ] **v0.6** — 插件系统（协议包独立 JAR）
