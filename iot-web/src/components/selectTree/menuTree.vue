...
<template>
  <div class="w-full">
    <div v-for="item in data" :key="item.id" class="py-6px">
      <div
        class="flex flex-row items-center h-40px px-8px cursor-pointer" :class="[{ itemHover: item.permissionUrl !== actPath }, { act: item.permissionUrl === actPath }]"
      ...
      >
        ...
        <el-icon v-if="item.children.length" :size="12" class="down" :class="[{ downAct: item.showChild }]">
          <ArrowDownBold />
        </el-icon>
      </div>
      <div class="w-full">
        <el-collapse-transition>
          <div v-if="item.children && item.showChild">
            <menuTree :data="item.children" :act-path="actPath" />
          </div>
        </el-collapse-transition>
      </div>
    </div>
